...
// Bolt: Using std::string_view prevents unnecessary copying and allocations when trimming whitespace from read-only strings.
...
bool comment_heading(const std::string &line, std::string *heading) {
  ...
  if (offset + 1 < line.size() && line[offset] == '/' && line[offset + 1] == '/')
    ...
    return false;
  std::string_view value = trim_copy(std::string_view(line).substr(offset));
