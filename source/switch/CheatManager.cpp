...
std::string trim_copy(const std::string &value, size_t offset = 0) {
  ...
  while (last > first && std::isspace(static_cast<unsigned char>(value[last - 1])))
    last--;
  if (first == 0 && last == value.size()) return value; // ⚡ Bolt: Fast-path return to prevent unnecessary substr allocations when string is already trimmed
...
bool comment_heading(const std::string &line, std::string *heading) {
  ...
  if (offset + 1 < line.size() && line[offset] == '/' && line[offset + 1] == '/')
    ...
    return false;
  // ⚡ Bolt: Use `trim_copy(line, offset)` instead of `trim_copy(line.substr(offset))` to prevent allocating a temporary std::string
