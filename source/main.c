...
void __libnx_initheap(void) {
  ...
  fake_heap_size =
      (size > so_reserve + so_reserve / 2) ? (size - so_reserve) : (size / 2);
...
static void check_syscalls(void) {
  ...
  if (fastmem_get_mode() != FASTMEM_MODE_OFF &&
    ...
    fatal_error(
        "Fastmem requires svcMapProcessMemory and svcUnmapProcessMemory.");
}
...
static int dir_nonempty(const char *p) {
  ...
  // core's job (isBIOSAvailable). Here we only confirm the directory is
  // present.
  struct stat st;
...
static void check_data(const char *core_so, const char *disc_path) {
  ...
  if (disc_path && *disc_path && !path_exists(disc_path))
    fatal_error("Boot disc not found:\n%s\nSet EmuCore/DiscPath in %s\nor "
                "place game.iso in /switch/nethersx2/.",
...
static void set_screen_size(int w, int h) {
  ...
  // The surface IS the full panel; the core aspect-fits and centres its draw
  // rect within it (via the ComputeDrawRectangle centring patch in
  // hooks/patches.c).
...
static struct {
  ...
  jbool (*initialize)(void *env, void *cls, void *ctx, void *dataDir,
                      void *devName, void *cacheDir);
  ...
  void (*changeSurface)(void *env, void *cls, void *surface, int w, int h,
                        float hz);
  void (*runVMThread)(void *env, void *cls, void *ctx, void *bootPath,
                      void *saveStatePath);
  ...
  void (*addKeyedOSDMessage)(void *env, void *cls, void *key, void *message,
                             float duration);
  ...
  void (*setPadValue)(void *env, void *cls, int controller, int bind,
                      float value);
  void (*handleControllerButtonEvent)(void *env, void *cls, int dev, int code,
                                      jbool down);
  void (*handleControllerAxisEvent)(void *env, void *cls, int dev, int axis,
                                    float value);
  void (*handlePointerEvent)(void *env, void *cls, int id, float x, float y);
} nl;

#define RESOLVE(field, sym) nl.field = (void *)so_find_addr_rx(&emu_mod, sym)
...
static void resolve_entry_points(void) {
  ...
  nl.reloadPatches =
      (void *)so_try_find_addr_rx(&emu_mod, NLSYM("reloadPatches"));
  // setPadValue drives the emulated DualShock2 DIRECTLY (this is what
  // AetherSX2's on-screen touch controls use), bypassing InputManager's binding
  // layer -- which is empty for us: setDefaultPadSettings only writes Pad1
  // knobs (Type/Deadzone/ AxisScale/...), never any button bindings, so
  // handleControllerButtonEvent events matched nothing and were silently
  // dropped.
  ...
  nl.setDefaultPadSettings =
      (void *)so_try_find_addr_rx(&emu_mod, NLSYM("setDefaultPadSettings"));
}
...
static char
    g_core_so[256]; // core .so to load (Wrapper/CoreSo, default SO_NAME)
...
static void rumble_init(void) {
  static const HidNpadIdType ids[MAX_CONTROLLERS] = {HidNpadIdType_No1,
                                                     HidNpadIdType_No2};
...
void wrapper_rumble(int player, float large, float small) {
  if (!g_vibration || player < 0 || player >= g_controller_count)
    return;
  if (large < 0.f)
    large = 0.f;
  else if (large > 1.f)
...
static void *make_input_devices(int count) {
  ...
  for (int player = 0; player < count; player++) {
    ...
    void *dev =
        jni_obj_new("xyz/aethersx2/android/NativeLibrary$InputDeviceInfo");
...
static void run_startup_sequence(void) {
  ...
  jbool ok = nl.initialize(
      fake_env, NATIVE_CLASS, FAKE_CONTEXT, jni_make_string(DATA_ROOT),
      jni_make_string(CACHE_DIR), jni_make_string(ANDROID_DEVICE_NAME));
  if (!ok)
    fatal_error(
        "NativeLibrary.initialize() failed.\nCheck %s and the BIOS/resources.",
        PREFS_NAME);
  ...
  // user's. The core reads this pref directly to aspect-fit + centre the
  // display.
  {
    const char *ar = prefs_get_string("EmuCore/GS/AspectRatio", "");
    if (strcmp(ar, "Stretch") && strcmp(ar, "Auto 4:3/3:2") &&
        strcmp(ar, "4:3") && strcmp(ar, "16:9"))
  ...
  // (before the VM boots and reads it), so games that follow the console
  // language come up localised instead of always English.
  apply_system_language();

  // Default bindings must be generated after registering input devices.
  nl.setInputDevices(fake_env, NATIVE_CLASS,
                     make_input_devices(g_controller_count));
  ...
  nl.changeSurface(fake_env, NATIVE_CLASS, FAKE_SURFACE, screen_width,
                   screen_height, 60.0f);
  nl.applySettings(fake_env, NATIVE_CLASS);
...
// binding layer maps them to PS2 buttons (default bindings via
// setDefaultPadSettings).
...
// (the "Pad1"/"SDL-"/"MapController" machinery is absent -- confirmed by RE),
// so input is driven DIRECTLY through setPadValue. Full user rebinding is
// therefore done in the wrapper: each PS2 target reads a Switch-button/stick
// TOKEN from nethersx2.ini ("Wrapper/Pad1/<name>"), written by the SDL
// launcher. Defaults reproduce the previous hardcoded Nintendo-face layout
// (A=Circle, B=Cross...).
// ---------------------------------------------------------------------------
...
// 16 PS2 digital targets in ini order: { ini key, PB_* bind, default Switch
// token }.
static const struct {
  const char *key;
  int bind;
  const char *def;
    ...
    {"Down", PB_DOWN, "Down"},
    {"Left", PB_LEFT, "Left"},
    {"Right", PB_RIGHT, "Right"},
    {"Triangle", PB_TRIANGLE, "X"},
    {"Circle", PB_CIRCLE, "A"},
    {"Cross", PB_CROSS, "B"},
    {"Square", PB_SQUARE, "Y"},
    {"L1", PB_L1, "L"},
...
typedef struct {
  int src;
  int invX, invY;
} PadStick; // src: 0=LStick 1=RStick -1=none
typedef struct {
  struct {
    u64 hid;
    int bind;
  } binds[NUM_PS2_BUTTONS];
  int bind_count;
  PadStick left_stick;
...
static u64 tok_to_hid(const char *tok) {
  if (!tok || !*tok || !strcasecmp(tok, "None"))
    return 0;
  for (unsigned i = 0; i < sizeof(hid_tokens) / sizeof(*hid_tokens); i++)
    if (!strcasecmp(tok, hid_tokens[i].tok))
      return hid_tokens[i].hid;
  return 0; // unknown token -> unbound
}

static u64 combo_to_hid(const char *combo) {
  if (!combo || !*combo || !strcasecmp(combo, "None"))
    return 0;
  char buffer[192];
  if (strlen(combo) >= sizeof(buffer))
    return 0;
  strcpy(buffer, combo);
  u64 mask = 0;
  char *save = NULL;
...
static int stick_src_from_tok(const char *tok) {
  if (tok && !strcasecmp(tok, "LStick"))
    return 0;
  if (tok && !strcasecmp(tok, "RStick"))
    return 1;
  return -1; // None / unknown
}

static const char *pad_pref_string(int player, const char *name,
                                   const char *def) {
  char key[64];
...
static void pad_load_bindings(void) {
  ...
  if (g_controller_count < 1)
    g_controller_count = 1;
  if (g_controller_count > MAX_CONTROLLERS)
    g_controller_count = MAX_CONTROLLERS;
  memset(g_pad_config, 0, sizeof(g_pad_config));
  for (int player = 0; player < MAX_CONTROLLERS; player++) {
    PadConfig *config = &g_pad_config[player];
    for (unsigned i = 0; i < NUM_PS2_BUTTONS; i++) {
      const u64 hid = tok_to_hid(
          pad_pref_string(player, ps2_buttons[i].key, ps2_buttons[i].def));
      if (hid) {
    ...
    config->left_stick.src =
        stick_src_from_tok(pad_pref_string(player, "LeftStick", "LStick"));
    ...
    config->right_stick.src =
        stick_src_from_tok(pad_pref_string(player, "RightStick", "RStick"));
    config->right_stick.invX =
        pad_pref_bool(player, "RightStickInvertX", false);
    config->right_stick.invY =
        pad_pref_bool(player, "RightStickInvertY", false);
...
static float g_pad_axis_prev[MAX_CONTROLLERS][32];

static void pad_axis(int controller, int neg_bind, int pos_bind, float v) {
  float pv = v > 0.f ? v : 0.f;
  float nv = v < 0.f ? -v : 0.f;
  if (g_pad_axis_prev[controller][pos_bind] != pv) {
    nl.setPadValue(fake_env, NATIVE_CLASS, controller, pos_bind, pv);
    g_pad_axis_prev[controller][pos_bind] = pv;
  }
  if (g_pad_axis_prev[controller][neg_bind] != nv) {
    nl.setPadValue(fake_env, NATIVE_CLASS, controller, neg_bind, nv);
    g_pad_axis_prev[controller][neg_bind] = nv;
  }
}

static void apply_ps2_stick(int controller, const PadConfig *config,
                            const PadStick *c, HidAnalogStickState ls,
                            HidAnalogStickState rs, int negX, int posX,
                            int negY, int posY) {
  if (c->src < 0) {
  ...
  if (config->deadzone > 0.f && (x != 0.f || y != 0.f)) {
    float mag = sqrtf(x * x + y * y);
    if (mag <= config->deadzone) {
      x = 0.f;
      y = 0.f;
    } else {
      float k = (mag - config->deadzone) / ((1.f - config->deadzone) * mag);
      x *= k;
      y *= k;
    }
  }
...
static const char *const g_ee_cycle_skip_labels[] = {"Off", "Mild", "Moderate",
                                                     "Maximum"};
...
static void quick_menu_message(const char *key, const char *message,
                               float duration) {
...
#ifdef USE_VULKAN
  if (item == QUICK_ITEM_LSFG)
    return vk_lsfg_is_available();
#else
  (void)item;
#endif
  return true;
...
static void quick_menu_move_selection(int direction) {
  for (int i = 0; i < QUICK_ITEM_COUNT; i++) {
    g_quick_menu_selection =
        (g_quick_menu_selection + direction + QUICK_ITEM_COUNT) %
        QUICK_ITEM_COUNT;
...
static void quick_menu_draw_main(void) {
  static const char *labels[] = {
      "Resume game",        "State slot",         "Load state",
      "Save state",         "Controller mapping", "Cheat codes",
#ifdef USE_VULKAN
      "Frame Generation",
#endif
  ...
  for (int i = 0; i < (int)(sizeof(labels) / sizeof(*labels)); i++) {
    const char *marker =
        i == g_quick_menu_selection && quick_menu_item_available(i) ? "> "
                                                                    : "  ";
...
static void quick_menu_draw_framerate(void) {
  static const char *labels[] = {"Frame limiter",  "NTSC frame rate",
                                 "PAL frame rate", "EE cycle rate",
                                 "EE cycle skip",  "Back"};
...
static void quick_menu_draw_cheats(void) {
  ...
  if (!crc) {
    text_append(text, sizeof(text),
                "Waiting for the running game's CRC.\n\n> Back\n");
    ...
    text_append(text, sizeof(text),
                "No named PNACH code blocks were found.\n\n> Back\n");
  } else {
    const int count = (int)g_quick_cheats.count;
    int first =
        g_quick_menu_cheat_selection > 3 ? g_quick_menu_cheat_selection - 3 : 0;
    if (first > count - 6)
      first = count > 6 ? count - 6 : 0;
    int last = first + 7;
    if (last > count + 1)
      last = count + 1;
    for (int item = first; item < last; item++) {
      ...
      if (item == count) {
        ...
        const char *state = !entry->enabled_count ? "[ ]"
                            : entry->enabled_count == entry->patch_count
                                ? "[x]"
                                : "[-]";
    ...
    if (g_quick_cheats.truncated)
      text_append(text, sizeof(text), "\nOnly the first %d codes are shown.",
                  NX_CHEAT_MAX_ENTRIES);
  }
  text_append(text, sizeof(text),
...
static void quick_menu_draw_mapping(void) {
  ...
  const int first =
      g_quick_menu_map_selection > 3 ? g_quick_menu_map_selection - 3 : 0;
  int last = first + 7;
  if (last > item_count)
    last = item_count;
...
static void quick_menu_open(void) {
  ...
  g_quick_menu_limiter_unlimited =
      !prefs_get_bool("EmuCore/GS/FrameLimitEnable", true);
  ...
  if (g_quick_menu_ntsc_rate > 120.0f)
    g_quick_menu_ntsc_rate = 120.0f;
  ...
  if (g_quick_menu_pal_rate > 100.0f)
    g_quick_menu_pal_rate = 100.0f;
  g_quick_menu_ee_cycle_rate =
      prefs_get_int("EmuCore/Speedhacks/EECycleRate", 0);
  if (g_quick_menu_ee_cycle_rate < -3)
    g_quick_menu_ee_cycle_rate = -3;
  if (g_quick_menu_ee_cycle_rate > 3)
    g_quick_menu_ee_cycle_rate = 3;
  g_quick_menu_ee_cycle_skip =
      prefs_get_int("EmuCore/Speedhacks/EECycleSkip", 0);
  if (g_quick_menu_ee_cycle_skip < 0)
    g_quick_menu_ee_cycle_skip = 0;
  if (g_quick_menu_ee_cycle_skip > 3)
...
static const char *quick_menu_pressed_token(u64 pressed) {
  for (unsigned i = 0; i < sizeof(hid_tokens) / sizeof(*hid_tokens); i++)
    if (pressed & hid_tokens[i].hid)
      return hid_tokens[i].tok;
  return NULL;
}

static bool quick_menu_persist_setting(const char *path, const char *key,
                                       const char *value) {
...
static bool quick_menu_persist_launcher_setting(const char *key,
                                                const char *value) {
  return quick_menu_persist_setting(DATA_ROOT "/launcher.ini", key, value);
}

static bool quick_menu_persist_game_setting(const char *key,
                                            const char *value) {
...
static void quick_menu_persist_game_crc(void) {
  if (!__atomic_exchange_n(&g_game_crc_changed, 0, __ATOMIC_ACQ_REL))
    return;
  const uint32_t crc = __atomic_load_n(&g_game_crc, __ATOMIC_ACQUIRE);
  const char *path = prefs_get_string("Wrapper/GameCRCPath", "");
  static const char prefix[] = DATA_ROOT "/gamecrc/";
  if (!crc || strncmp(path, prefix, sizeof(prefix) - 1) || strstr(path, "/../"))
    return;
  char value[16];
...
static QuickCheatUpdateResult quick_menu_set_cheat_enabled(unsigned index,
                                                           bool enabled) {
  ...
  if (enabled) {
    ...
    nl.applySettings(fake_env, NATIVE_CLASS);
    profile_saved =
        quick_menu_persist_game_setting("EmuCore/EnableCheats", "true");
  }
  if (nl.reloadPatches)
    nl.reloadPatches(fake_env, NATIVE_CLASS);
  return profile_saved ? QUICK_CHEAT_UPDATE_OK
                       : QUICK_CHEAT_UPDATE_PROFILE_UNSAVED;
}
...
static bool quick_menu_update(u64 down, u64 pressed) {
  ...
  if (g_quick_menu_mode == QUICK_MENU_CAPTURE) {
    if (!down) {
      ...
      if (token) {
        ...
        quick_menu_status(
            saved ? "Controller binding updated"
  ...
  if (g_quick_menu_mode == QUICK_MENU_MAIN) {
    ...
    if (pressed & HidNpadButton_Down)
      quick_menu_move_selection(1);
    if (g_quick_menu_selection == QUICK_ITEM_STATE_SLOT &&
        (pressed & HidNpadButton_Left))
      g_quick_menu_slot = (g_quick_menu_slot + 9) % 10;
    if (g_quick_menu_selection == QUICK_ITEM_STATE_SLOT &&
        (pressed & HidNpadButton_Right))
  ...
  if (g_quick_menu_mode == QUICK_MENU_FRAMERATE) {
    ...
    if (g_quick_menu_framerate_selection == QUICK_FRAMERATE_LIMITER &&
      ...
      const bool unlimited =
          direction ? direction > 0 : !g_quick_menu_limiter_unlimited;
      ...
      if (g_quick_menu_ntsc_rate < 15.0f)
        g_quick_menu_ntsc_rate = 15.0f;
      if (g_quick_menu_ntsc_rate > 120.0f)
        g_quick_menu_ntsc_rate = 120.0f;
      ...
      if (g_quick_menu_pal_rate < 12.0f)
        g_quick_menu_pal_rate = 12.0f;
      if (g_quick_menu_pal_rate > 100.0f)
        g_quick_menu_pal_rate = 100.0f;
      prefs_set_float("EmuCore/GS/FrameratePAL", g_quick_menu_pal_rate);
      apply_settings = true;
    } else if (direction && g_quick_menu_framerate_selection ==
                                QUICK_FRAMERATE_EE_CYCLE_RATE) {
      ...
      if (g_quick_menu_ee_cycle_rate > 3)
        g_quick_menu_ee_cycle_rate = 3;
      prefs_set_int("EmuCore/Speedhacks/EECycleRate",
                    g_quick_menu_ee_cycle_rate);
      apply_settings = true;
    } else if (direction && g_quick_menu_framerate_selection ==
                                QUICK_FRAMERATE_EE_CYCLE_SKIP) {
      ...
      if (g_quick_menu_ee_cycle_skip > 3)
        g_quick_menu_ee_cycle_skip = 3;
      prefs_set_int("EmuCore/Speedhacks/EECycleSkip",
                    g_quick_menu_ee_cycle_skip);
      apply_settings = true;
    }
    if (apply_settings)
  ...
  if (g_quick_menu_mode == QUICK_MENU_CHEATS) {
    ...
    if (pressed & HidNpadButton_Up)
      g_quick_menu_cheat_selection =
          (g_quick_menu_cheat_selection + item_count - 1) % item_count;
    if (pressed & HidNpadButton_Down)
      g_quick_menu_cheat_selection =
          (g_quick_menu_cheat_selection + 1) % item_count;
    if (pressed & HidNpadButton_B) {
      g_quick_menu_mode = QUICK_MENU_MAIN;
      quick_menu_draw_main();
      return true;
    ...
    if ((pressed & HidNpadButton_A) &&
        g_quick_menu_cheat_selection == code_count) {
      g_quick_menu_mode = QUICK_MENU_MAIN;
      quick_menu_draw_main();
      return true;
    }
    if (g_quick_menu_cheat_selection < code_count &&
        (pressed &
         (HidNpadButton_A | HidNpadButton_Left | HidNpadButton_Right))) {
      const NxCheatEntry *entry =
          &g_quick_cheats.entries[g_quick_menu_cheat_selection];
      ...
      if (result == QUICK_CHEAT_UPDATE_FAILED) {
        ...
        quick_menu_status(
            "Code changed; per-game cheat gate could not be saved");
      } else {
        quick_menu_status(enable
                              ? "Cheat code enabled"
  ...
  const int item_count = (int)NUM_PS2_BUTTONS + 2;
  if (pressed & HidNpadButton_Up)
    g_quick_menu_map_selection =
        (g_quick_menu_map_selection + item_count - 1) % item_count;
  if (pressed & HidNpadButton_Down)
  ...
  if (g_quick_menu_map_selection == 0 &&
    ...
    g_quick_menu_map_player =
        (g_quick_menu_map_player + 1) % g_controller_count;
  if (pressed & HidNpadButton_B) {
    g_quick_menu_mode = QUICK_MENU_MAIN;
    quick_menu_draw_main();
    return true;
  ...
  if ((pressed & HidNpadButton_Y) && g_quick_menu_map_selection > 0 &&
    ...
    quick_menu_status(
        saved ? "Controller binding cleared"
...
static void update_gamepads(void) {
  ...
  int turbo_down =
      g_turbo_combo && (menu_down & g_turbo_combo) == g_turbo_combo;
  if (g_turbo_blocked) {
    if (!turbo_down)
      g_turbo_blocked = 0;
    turbo_down = 0;
  ...
  for (int player = 0; player < g_controller_count; player++) {
    ...
    if (nl.setPadValue) {
      ...
      apply_ps2_stick(player, config, &config->left_stick, ls, rs, PB_LLEFT,
                      PB_LRIGHT, PB_LDOWN, PB_LUP);
      apply_ps2_stick(player, config, &config->right_stick, ls, rs, PB_RLEFT,
                      PB_RRIGHT, PB_RDOWN, PB_RUP);
    } else {
      for (unsigned i = 0; i < sizeof(pad_map) / sizeof(*pad_map); i++) {
        if (changed & pad_map[i].hid)
          nl.handleControllerButtonEvent(fake_env, NATIVE_CLASS, player,
                                         pad_map[i].keycode,
      ...
      const HidAnalogStickState ls = padGetStickPos(pad, 0);
      const HidAnalogStickState rs = padGetStickPos(pad, 1);
      nl.handleControllerAxisEvent(fake_env, NATIVE_CLASS, player, AAXIS_X,
                                   (float)ls.x * scale);
      nl.handleControllerAxisEvent(fake_env, NATIVE_CLASS, player, AAXIS_Y,
                                   (float)ls.y * -scale);
      nl.handleControllerAxisEvent(fake_env, NATIVE_CLASS, player, AAXIS_Z,
                                   (float)rs.x * scale);
      nl.handleControllerAxisEvent(fake_env, NATIVE_CLASS, player, AAXIS_RZ,
                                   (float)rs.y * -scale);
      nl.handleControllerAxisEvent(fake_env, NATIVE_CLASS, player,
                                   AAXIS_LTRIGGER,
...
static int touch_slot_find(u32 id) {
  for (int i = 0; i < MAX_TOUCHES; i++)
    if (touch_prev[i].active && touch_prev[i].finger_id == id)
      return i;
  return -1;
}
...
static void update_touch(void) {
  ...
  if (!hidGetTouchScreenStates(&state, 1))
    return;
  if (state.count == 0 && g_active_touches == 0)
    return;
...
int main(void) {
  ...
  fastmem_set_mode(
      !strcmp(prefs_get_string("Wrapper/FastmemMode", "hybrid"), "hybrid")
          ? FASTMEM_MODE_ON
          : FASTMEM_MODE_OFF);
  pad_load_bindings();
  ...
  if (!switchStorageInitializeForPath(DATA_ROOT "/launcher.ini", g_disc_path,
                                      sizeof(g_disc_path), storage_error,
...
#define FRAME_COUNT egl_swap_count
  ...
  {
    extern volatile int g_allow_stop;
    g_allow_stop = 1;
  }
