...
static void stringlist_add(const char *key, const char *val, int add) {
  ...
  // explicit length tracking avoids O(N^2) concatenation
  size_t out_len = 0;
  char *tmp = strdup(cur);
  char *save = NULL;
...
static void dispatch_void(const char *name, va_list va) {
  ...
  if (!strcmp(name, "setStringList")) {
    ...
    for (int i = 0; i < n; i++) {
      if (i > 0) out[out_len++] = '\n';
      size_t tok_len = strlen(obj_str(jni_obj_array_get(jarr, i)));
      memcpy(out + out_len, obj_str(jni_obj_array_get(jarr, i)), tok_len);
      out_len += tok_len;
    }
    out[out_len] = '\0';
