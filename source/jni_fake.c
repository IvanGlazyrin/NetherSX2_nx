...
static void stringlist_add(const char *key, const char *val, int add) {
  ...
  char *out = malloc(cap); // allocate without zeroing since we track length explicitly
  if (out) out[0] = '\0';
  size_t out_len = 0;
  char *tmp = strdup(cur);
  char *save = NULL;
  for (char *tok = strtok_r(tmp, "\n", &save); tok; tok = strtok_r(NULL, "\n", &save)) {
    ...
    // ⚡ Bolt: Maintain string length and copy via memcpy to prevent O(N^2) strncat scanning
...
static void dispatch_void(const char *name, va_list va) {
  ...
  if (!strcmp(name, "setStringList")) {
    ...
    size_t out_len = 0;
    for (int i = 0; i < n; i++) {
      const char* item_str = obj_str(jni_obj_array_get(jarr, i));
      size_t item_len = strlen(item_str);

      // ⚡ Bolt: Track string length manually and use memcpy instead of O(N^2) strncat
      if (i > 0) out[out_len++] = '\n';
      memcpy(out + out_len, item_str, item_len);
      out_len += item_len;
      out[out_len] = '\0';
    }
