...
static void stringlist_add(const char *key, const char *val, int add) {
  ...
  // ⚡ Bolt Optimization: Build strings in O(N) by manually tracking the length,
  // rather than calling strncat + strlen which results in O(N²) running time.
  // Impact: Reduces time complexity of rebuilding string lists from O(N²) to O(N).
  size_t out_len = 0;
  char *tmp = strdup(cur);
  char *save = NULL;
